...
func disconnect_game(error = 0):
	...
	if is_server:
		peer.stop()
		if peer.is_connected("client_connected",Callable(self,"_peer_connected")):
			peer.disconnect("client_connected",Callable(self,"_peer_connected"))
		if peer.is_connected("client_disconnected",Callable(self,"_peer_disconnected")):
